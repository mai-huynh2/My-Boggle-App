...
function App() {
  ...
  const startNewGame = async () => {
    ...
    try {
      const res = await fetch(`${BASE_URL}/api/game/${size}/?language=${language}`);
  ...
  const submitScore = async () => {
    ...
    try {
      const response = await fetch(`${BASE_URL}/api/games/${game.id}/leaderboard/`, {
        method: "POST",
        headers: {
  ...
  return (
    <div style={{ textAlign: "center" }}>
      ...
      {gameState === GAME_STATE.IN_PROGRESS && <h2>Time Left: {timeLeft}s</h2>}
      ...
      {gameState === GAME_STATE.BEFORE && <Leaderboard onSelectGame={loadExistingGame} />}
      ...
      {gameState === GAME_STATE.IN_PROGRESS && (
        <>
          ...
          <FoundSolutions headerText="Correct Guesses" words={foundSolutions} />
        </>
      )}

      {gameState === GAME_STATE.ENDED && (
        <>
          <SummaryResults words={foundSolutions} totalTime={selectedTime - timeLeft} />
          ...
          <button onClick={submitScore} disabled={hasSubmitted || !playerName.trim()}>
